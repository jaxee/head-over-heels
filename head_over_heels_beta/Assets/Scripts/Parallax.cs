...
public class Parallax : MonoBehaviour {
	...
	private Transform camTrans;	//reference to the camera's transform
	...
	void Start () {
		...
		previousCamPos = camTrans.position;
	...
	void Update() {
		...
		for (int i = 0; i < backgrounds.Length; i++) {
			...
			float parallax =  (previousCamPos.x - camTrans.position.x) * parallaxScales[i];
